...
class Dataset_CRNN(data.Dataset):
    "Characterizes a dataset for PyTorch"
    def __init__(self, data_path, videos, labels, class_names, frames, transform=None, is_cropped=None):
        "Initialization"
        self.data_path = data_path
        self.videos = videos
        self.labels = labels
    ...
    def read_images(self, path, selected_folder, video_name, use_transform):
        X = []
        cap = cv2.VideoCapture(os.path.join(path,selected_folder,video_name))
    ...
    def __getitem__(self, index):
        ...
        folder = self.class_names[self.labels[index]]

        # Load data
        X = self.read_images(self.data_path, folder, video, self.transform)     # (input) spatial images
        y = torch.LongTensor([self.labels[index]])                  # (labels) LongTensor are for int64 instead of FloatTensor
